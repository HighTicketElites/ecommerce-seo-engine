...
import manifest from '../data/hovbeta-manifest.js';
import body01 from '../data/hovbeta-body-01.js';
import body02 from '../data/hovbeta-body-02.js';
import body03 from '../data/hovbeta-body-03.js';
import body04 from '../data/hovbeta-body-04.js';
import body05 from '../data/hovbeta-body-05.js';
import body06 from '../data/hovbeta-body-06.js';
import body07 from '../data/hovbeta-body-07.js';

const sourceBody = [body01,body02,body03,body04,body05,body06,body07].join('');
const PRODUCT_HANDLE = 'hovsco™-hovbeta-20-foldable-fat-tire-electric-bike';
...
function chooseImages(product) {
  ...
  if(!all.length) throw new Error('No authentic Shopify product images found for HOVSCO HovBeta');
...
function buildBody(product,images){
  ...
  if(price?.amount) body=body.replace(/At the time of this review, WattWheelz lists the HovBeta from \$1,499/g,`At the time of this review, WattWheelz lists the HovBeta from $${Number(price.amount).toLocaleString('en-US',{minimumFractionDigits:0,maximumFractionDigits:2})}`);
  body=body.replace('<!-- IMAGE 1: Authentic current HOVSCO HovBeta full-bike Shopify image. -->',imgFigure(images.fullBike,'HOVSCO HovBeta folding fat-tire electric bike side profile','Current HOVSCO HovBeta folding e-bike product image.'));
  body=body.replace('<!-- IMAGE 2: Authentic current HOVSCO HovBeta Shopify product detail image. -->',imgFigure(images.detail1,'HOVSCO HovBeta folding e-bike product detail','Authentic HOVSCO HovBeta product detail.'));
  body=body.replace('<!-- IMAGE 3: Authentic current HOVSCO HovBeta alternate Shopify image. -->',imgFigure(images.detail2,'HOVSCO HovBeta folding fat-tire e-bike','Authentic HOVSCO HovBeta alternate product image.'));
  return body;
}
...
export async function runDraftJob(auth){
  ...
  if(product.status!=='ACTIVE') throw new Error(`HovBeta product status is ${product.status}; draft generation stopped`);
  ...
  const existing=matches[0]||null; if(existing?.isPublished) throw new Error('Existing HovBeta review is published; draft generator refuses to modify a live article');
  const images=chooseImages(product); const body=buildBody(product,images);
  const articleInput={blogId:blog.id,title:manifest.title,handle:manifest.handle,body,summary:manifest.excerpt,tags:manifest.tags,author:{name:'WattWheelz'},isPublished:false,image:{url:images.featured.url,altText:'HOVSCO HovBeta folding fat-tire electric bike'},metafields:metafields(existing)};
  let action;
